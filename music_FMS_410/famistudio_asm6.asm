...
FAMISTUDIO_ASM6_ZP_ENUM   = $00F9 ; 7 bytes
FAMISTUDIO_ASM6_BSS_ENUM  = $6000 ; prg-ram/sram
;FAMISTUDIO_ASM6_CODE_BASE = $CC00
...
fms_channels	          .dsb 5 ; 2a03 apu channels ctrl
epsm_square_channels_ctrl .dsb 3 ; epsm
epsm_fm_channels_ctrl	  .dsb 6 ; epsm
epsm_rhythm_channel_ctrl  .dsb 6 ; epsm

famistudio_sfx_instrument_lo .dsb 1
famistudio_sfx_instrument_hi .dsb 1
...
famistudio_epsm_register_order1:
    .byte $40, $30, $B4, $B0
famistudio_epsm_register_order2:
    .byte $50, $60, $70, $80, $90
    .byte $38, $48, $58, $68, $78, $88, $98
    .byte $34, $44, $54, $64, $74, $84, $94
    .byte $3C, $4C, $5C, $6C, $7C, $8C, $9C

famistudio_epsm_channel_key_table:
    .byte $f0, $f1, $f2, $f4, $f5, $f6
famistudio_epsm_channel_key_table_andF:
...
@vol_off:
    TYA
    CLC
    ADC	#FAMISTUDIO_EPSM_REG_TL ; $40-$4F
    STA	FAMISTUDIO_EPSM_ADDR,X
    LDA	#$7F
    STA	FAMISTUDIO_EPSM_DATA,X
    JSR	@wait
    DEY
    BPL	@vol_off
@wait
...
.macro famistudio_update_channel_sound idx, env_offset, pulse_prev, reg_hi, reg_lo, reg_vol, reg_sweep, phase_reset_mask
    ...
    lda fms_channels+idx
    beq pro_channel
    ;lda #$FF	; needs only for pulse1 and 2
    ;sta pulse_prev+idx
    jmp skip_channel
...
no_noise_slide:
    ...
    ora famistudio_noise_duty_lookup, x
...
famistudio_update_epsm_square_channels_sound:
    
    pitch = famistudio_ptr1

    ldy #2 ; 3 channels
    
@next_epsm_square_channel:
    lda epsm_square_channels_ctrl,Y
    beq @pro_epsm_square
    dey
    bpl @next_epsm_square_channel
    jmp famistudio_update_epsm_fm_channel_sound
    
@pro_epsm_square
    ldx famistudio_chn_note+FAMISTUDIO_EPSM_CH0_IDX,y
    bne @nocut
...
@next_epsm_fm_channel:
    lda epsm_fm_channels_ctrl,y ; skip if not 0
    bne @done_epsm_fm_channel1
...
@cut:  
    ...
    lda famistudio_epsm_channel_key_table_andF, y
 ...
 @done_epsm_fm_channel1:   
    dey
    bpl @next_epsm_fm_channel
    ;jmp famistudio_update_epsm_rhythm_channel_sound
    rts ; rhythm channels not used in sonic ost
...
@done_epsm_fm_channel:
    dey
    bmi @done
    jmp @next_epsm_fm_channel
@done
    .if	(EPSM_RHYTM=0)
    ;rts
    ;jmp FAMISTUDIO_UPDATE_EPSM_RHYTHM_CHANNEL_SOUND
...
famistudio_update_epsm_rhythm_channel_sound:
    ...
    ldy	#5 ; 6 channels
@next_epsm_rhythm_channel:
    lda epsm_rhythm_channel_ctrl,y
    bne @noupdate
...
.endif
    ...
    bne @update
    rts	; music off, famistudio playback disabled
@pause:
.if !FAMISTUDIO_USE_FAMITRACKER_TEMPO
    lda #1
...
@store_counter:
    ...
    bne @store_frame_count1
    jmp @skip_frame
    
@store_frame_count1:
	ldx	speed_shoes_timer
	beq	@no_add_tempo
	clc
	adc	#1
@no_add_tempo:
...
@update_row:
    ldx #0
    @channel_loop:
        lda famistudio_chn_ptr_hi,x ; not used if ptr = 0
	beq @next_ch
...
.endif	
            ...
            jsr famistudio_update_channel
            beq @next_ch
            jsr famistudio_update_row_cont
	    jmp @next_ch
@dec_repeat    
            dec famistudio_chn_repeat,x
...
@env_set_repeat:
    ...
    cmp #$7F
    bne @env_next_store_ptr
    inc famistudio_env_repeat,x ; $80 - disable flag
...
.if FAMISTUDIO_EXP_EPSM
@update_epsm_sound:
        jsr famistudio_update_epsm_square_channels_sound
...
.macro famistudio_get_exp_inst_ptr_sfx
    ...
    rol ptr+1
    clc
    adc famistudio_sfx_instrument_lo
    sta ptr+0
    lda ptr+1
    and #3 ; 64*16 = $3FF
    adc famistudio_sfx_instrument_hi
    sta ptr+1
    ldy #0
...
@loop_main_patch:
        lda famistudio_epsm_register_order1,x ; 4
        ora reg_offset ; 3
        sta select ; 4
        lda (ptr),y ; 5
        sta write ; 4
...
@loop_extra_patch:
        txa	; 2
        ora famistudio_epsm_register_order2,y ; 4
        sta select ; 4
        lda (ex_patch),y ; 5
        sta write ; 4
        dey	; 2
...
famistudio_set_epsm_instrument:
   ...
   sty chan_idx
   cpy #FAMISTUDIO_EPSM_CHAN_FM_START+5
   bne famistudio_not_epsm_FM6
   famistudio_get_exp_inst_ptr_sfx
   jmp famistudio_set_epsm_instrument_FM1_FM5
   
famistudio_not_epsm_FM6:
    famistudio_get_exp_inst_ptr
famistudio_set_epsm_instrument_FM1_FM5:
    famistudio_set_exp_instrument
...
@continue:
   ...
   ; lda famistudio_channel_epsm_chan_table,x
        ...
        ldx chan_idx
	lda famistudio_channel_epsm_chan_table-FAMISTUDIO_EPSM_CHAN_FM_START,x
	sta reg_offset   
    
        lda #FAMISTUDIO_EPSM_REG_KEY
        sta FAMISTUDIO_EPSM_REG_SEL0
        lda famistudio_epsm_channel_key_table_andF-FAMISTUDIO_EPSM_CHAN_FM_START, x
        ;and #$0f ; remove trigger
        sta FAMISTUDIO_EPSM_REG_WRITE0
    ...
    ; Now we need to store the algorithm and 1st operator volume for later use
        lda (ptr),y
        and #$07
        sta famistudio_chn_epsm_alg-FAMISTUDIO_EPSM_CHAN_FM_START,x ;store algorithm
        iny
        lda (ptr),y
        sta famistudio_chn_epsm_fm_stereo-FAMISTUDIO_EPSM_CHAN_FM_START ,x
        iny
        iny
        lda (ptr),y
        sta famistudio_chn_epsm_vol_op1-FAMISTUDIO_EPSM_CHAN_FM_START,x
        dey
        dey
        dey
    ; Now if we are channels 1-3 then we use @reg_set_0, otherwise for 4-6 its reg set 1
    ;lda chan_idx
    cpx #FAMISTUDIO_EPSM_CH6_IDX
    bpl @reg_set_1
    ...
    @last_reg:
       ...
       ; tax
        ...
        ldy #6
        lda (ex_patch),y
        sta famistudio_chn_epsm_vol_op2-FAMISTUDIO_EPSM_CHAN_FM_START,x
        ldy #13
        lda (ex_patch),y
        sta famistudio_chn_epsm_vol_op3-FAMISTUDIO_EPSM_CHAN_FM_START,x
        ldy #20 
        lda (ex_patch),y
        sta famistudio_chn_epsm_vol_op4-FAMISTUDIO_EPSM_CHAN_FM_START,x
	rts
...
@opcode_invalid:
    ...
    dey
    lda (channel_data_ptr),y ; read last cmd
    and #$3f
@bad_opcode:
    jmp @bad_opcode
...
@opcode_halt_channel:
	...
	sta fms_channels,X
	lda #0
	sta famistudio_chn_ptr_hi,x ; disable reading
        ;ldx #FAMISTUDIO_NUM_CHANNELS-7 ; FM6  X=$D
	jsr clear_channel_vars
	ldx #FAMISTUDIO_NUM_CHANNELS-7 ; FM6  X=$D
	rts

@opcode_end_song:
@opcode_halt:		; end of music track    
@ft_cmd_halt    
		LDA	current_music
;		CMP	#$2D
;		BEQ	set_lvl_music
...
@no_set_lvl_music:
		jsr famistudio_music_stop
		ora #$80 ; TODO : Should we stop, or just call famistudio_music_stop here?
		sta famistudio_song_speed
		pla
		pla
		rts
...
; Duty lookup table noise.
famistudio_noise_duty_lookup:
    .byte $00
    .byte $80
    .byte $00
    .byte $80 
...
INIT_FM6_SFX:
    ...
    lda sfx_instr_ptrs,Y
    sta famistudio_sfx_instrument_lo
    lda sfx_instr_ptrs+1,Y
    sta famistudio_sfx_instrument_hi

    lda #0
    cpy	#4*2 ; rings drop (fm)
    bcs @no_reset_std_sfx
    sta sfx_to_play
@no_reset_std_sfx
    sta fms_channels,x ; FM6 enabled
...
clear_channel_vars:
    ...
    ldx #$21 ; volume envelope FM6
    jsr	fm6_init_envelopes
    ldx #$22 ; arp envelope FM6
...
sfx_instr_ptrs:
    .WORD sfx_ring_inst
    .WORD sfx_badnik_inst
    .WORD sfx_spikes_inst
    .WORD sfx_spring_inst
    .WORD sfx_ring_drop_inst
    .WORD sfx_spin_inst
    
sfx_ring:
 @song0ch8:
@song0ch8loop:
	db $80, $35, $85, $38, $85, $7f, $50, $fb, $30, $3d, $bb, $5a, $8d, $00, $ff ; , $df ; , $42
	;dw @song0ch8loop
